...
const MetricCard = ({
  ...
  const CardContent = (
    <Box className="glass-card p-6 h-full flex flex-col justify-between hover:scale-[1.02] transition-transform duration-300 cursor-pointer">
  ...
  if (symbol) {
    return (
      <Link to={`/stocks/${symbol}`} style={{ textDecoration: 'none', color: 'inherit' }}>
        {CardContent}
      </Link>
    );
  }
